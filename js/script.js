$(document).ready(function() {
  $('#heartBtn').click(function(event) {
    event.stopPropagation();
    $('#registrationBox').toggle();
  });
  
  $('#registerOption').click(function(event) {
    event.stopPropagation();
  ...
  $(document).on('click', function(event) {
    ...
    var registrationOptions = $('#registrationOptions');
    
    // Check if the clicked element is not within the registration box or the heart button
    if (!target.is(registrationBox) && !target.is('#heartBtn') && !target.parents().is(registrationBox)) {
      registrationBox.hide();
  ...
  // Envio de datos en Formulario
  $('#registrationForm').submit(function(event) {
    event.preventDefault(); 
  
    var email = $('#email').val();
    var password = $('#password').val();
    var passwordRegex = /^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()])[A-Za-z\d!@#$%^&*()]{12,}$/;
  
    if (email === 'pabloz.gd@gmail.com') {
      if (passwordRegex.test(password)) {
        alert('Registrado!🔥');
        $('#registerForm').hide();
      } else {
        alert('La contraseña debe contener al menos 12 caracteres, incluyendo una letra mayúscula, un número y un símbolo.');
      }
    } else {
      alert('Corre inválido!. Por favor escriba el correo correctamente');
    }
  });
  $('#registerForm').prepend('<h2 class="form-title">Registro</h2>');
...
function createCartItem(productDetails) {
  ...
  <div class="checkout-cart__product" data-producttitle="${productDetails.title}">
        <img src="${productDetails.img}" alt="GPU" class="checkout-cart__img">
        <div class="checkout-cart__product-details">
            <h4 class="gpus__product-name">${productDetails.title}</h4>
            <p class="gpus__product-price">$${productDetails.price}</p>
            <br>
...
var contador = document.getElementById('contador-items');

// Define la cantidad inicial de items operativos
var cantidadItems = 0;
...
// Llama a la función de inicialización para configurar el contador inicialmente
actualizarContador();
...
function addTotalPrice() {
  ...
  cartItemCount.textContent = addedProductsToCart.length.toString();
}
...
// The Agregar al carrito button is under an extra div
